...
class GridWorld(object):
    def __init__(self, grid_world_size, walls, action_map, goal_dict, start_location, context,
        ...
        if sublvl_args == None:
            ...
            self.sublvl_locations = [l for _, l in list_sublvl_locations.iteritems()]
            ...
            for sublvl in range(self.n_sublvls):
                ...
                # reward to be updated to reflect sublevel states
                subgoal_dict = {l: (g, 1) for g, l in list_sublvl_locations.iteritems()}
                self.subgoal_dict = subgoal_dict
                
            for sublvl in range(self.n_sublvls):
                # to do: pdf for generating each sublvl
                sublvl_walls = []
                sublvl_start_loc = (0,0)
                self.sublvls[sublvl] = GridWorld(grid_world_size, sublvl_walls, sublvl_mappings[sublvl], subgoal_dict,
                                      sublvl_start_loc, sublvl, state_location_key=self.state_location_key)
    ...
    def move(self, key_press):
        ...
        if self.sublvl_state == self.n_sublvls and self.goal_check():
    ...
    def sublvl_check(self):
        if self.current_location == self.sublvl_locations[self.sublvl_state]:
            return True
        return False
