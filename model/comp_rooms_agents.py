...
class MultiStepAgent(object):
    ...
    def get_action_pmf(self, location, sublvl_state):
        assert type(location) is tuple
        return np.ones(self.task.n_primitive_actions, dtype=float) / self.task.n_primitive_actions

    def get_primitive_q(self, location, sublvl_state):
        return np.ones(self.task.n_primitive_actions)

    def get_action_cmf(self, location, sublvl_state):
        return np.cumsum(self.get_action_pmf(location, sublvl_state))
    ...
    def select_action(self, location, sublvl_state):
        return sample_cmf(self.get_action_cmf(location, sublvl_state))
    ...
    def generate(self, evaluate=True, debug=False, prunning_threshold=None):
        ...
        while True:
            ...
            start_sublvl_state = self.task.get_sublvl_state()
            action = self.select_action(start_location, start_sublvl_state)
            ...
            if debug:
                pmf = self.get_action_pmf(start_location, start_sublvl_state)
                p = pmf[action]
            else:
                p = None
            ...
            results_dict = {
                ...
                'Start Sublvl state': [start_sublvl_state],
                'Key-press': action,
...
class FlatAgent(MultiStepAgent):
    ...
    def get_abstract_action_q(self, location, sublvl_state):
        ...
        s = self.task.state_location_key[location] + sublvl_state*self.task.n_states
    ...
    def get_primitive_q(self, location, sublvl_state):
        q_aa = self.get_abstract_action_q(location, sublvl_state)
        c = self.task.get_current_context()
    ...
    def get_action_pmf(self, location, sublvl_state):
        ...
        q = self.get_abstract_action_q(location, sublvl_state)
...
class JointClusteringAgent(MultiStepAgent):
    ...
    def get_action_pmf(self, location, sublvl_state, threshold=0.01):
