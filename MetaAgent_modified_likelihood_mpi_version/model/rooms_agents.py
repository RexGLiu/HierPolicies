...
from cython_library import R_HierarchicalHypothesis as HierarchicalHypothesis
...
class HierarchicalAgent(MultiStepAgent):
    def __init__(self, task, inv_temp=10.0, 
                 alpha_r=1.0, alpha_hi=1.0, alpha_lo=1.0, 
                 gamma=0.8,
                 stop_criterion=0.001,
                 mapping_prior=0.001, goal_prior=0.001,
                 n_particles=300):
        super(HierarchicalAgent, self).__init__(task)
        ...
        self.rooms_hypotheses = [HierarchicalHypothesis(self.task.n_goals, 
                    ...
                    self.gamma, alpha_r, alpha_hi, alpha_lo, goal_prior, mapping_prior)]
    ...
    def update_mapping(self, c, a, aa):
        for ii, h_room in enumerate(self.rooms_hypotheses):
            ...
            h_room.update_mapping(c, a, self.task.abstract_action_key[aa])
    ...
    def resample_hypothesis_space(self):
        ...
        if n_beliefs > self.n_particles:
            self.log_belief = np.array(self.log_belief)
            self.rooms_hypotheses = np.array(self.rooms_hypotheses)

            # select the n_particles MAP hypotheses
            arg_idx = np.argpartition(self.log_belief, -self.n_particles)[-self.n_particles:]
            self.log_belief = self.log_belief[arg_idx]
            self.rooms_hypotheses = self.rooms_hypotheses[arg_idx]
