...
class Task(object):
    ...
    def __init__(self, list_start_location, list_end_location, list_context, list_action_map, list_goal_locations,
                 grid_world_size=(3, 3),
        ...
        self.list_goal_locations = list_goal_locations
...
def make_task(context_balance, context_goals, context_maps, hazard_rates, start_locations, grid_world_size, list_goal_locations,
              test_context_goals=None, test_context_maps=None, list_walls=None):
    
    if test_context_goals is None:
        return make_task_without_test(context_balance, context_goals, context_maps, hazard_rates, start_locations, grid_world_size, list_goal_locations, list_walls)
    else:
        return make_task_with_test(context_balance, context_goals, context_maps, hazard_rates, test_context_goals, test_context_maps, 
                                   start_locations, grid_world_size, list_goal_locations, list_walls)


def make_task_without_test(context_balance, context_goals, context_maps, hazard_rates, start_locations, grid_world_size, list_goal_locations,
               list_walls=None):
    ...
    args = [list_start_locations, list_goals, list_context, list_maps, list_goal_locations]
    kwargs = dict(list_walls=list_walls, grid_world_size=grid_world_size)
    return Task(*args, **kwargs)


def make_task_with_test(context_balance, context_goals, context_maps, hazard_rates, test_context_goals, test_context_maps, start_locations, grid_world_size, list_goal_locations,
              list_walls=None):

    train_context_balance = context_balance[:len(context_goals)]
    test_context_balance = context_balance[len(context_goals):]
    
    # start locations are completely random
    list_start_locations = list()
    for ctx, n_reps in enumerate(train_context_balance+test_context_balance):
        list_start_locations += [start_locations[np.random.randint(len(start_locations))] for _ in range(n_reps)]

    # use randomization function to shuffle contexts
    list_context = randomize_order(train_context_balance, hazard_rates)
    
    n_train_context = len(context_goals)
    list_test_context = [c+n_train_context for c,g in enumerate(test_context_goals) for ii in range(test_context_balance[c])]
    np.random.shuffle(list_test_context)
    
    list_context = list_context + list_test_context
    list_context_goals = context_goals + test_context_goals
    list_context_maps = context_maps + test_context_maps

    # list of goals and list of mappings depend on the context order
    list_goals = [list_context_goals[ctx] for ctx in list_context]
    list_maps = [list_context_maps[ctx] for ctx in list_context]
