...
def simulate_one(agent_class, simulation_number, task, agent_kwargs=None, pruning_threshold=1000,
    ...
    results, clusterings = agent.generate(pruning_threshold=pruning_threshold, evaluate=evaluate)
...
def simulate_mixed_task(n_sim, task_kwargs, agent_kwargs=None, alpha=2.0, pruning_threshold=1000,
    ...
    #     task = task_list[ii]
    #     task.reset()
    #     results_fl[ii], _ = simulate_one(FlatControlAgent, ii, task, pruning_threshold=pruning_threshold, agent_kwargs=agent_kwargs,
    #                                   evaluate=evaluate)

    for ii in tqdm(range(n_sim)):
        task = task_list[ii]
        task.reset()
        results_h[ii], _ = simulate_one(HierarchicalAgent, ii, task, agent_kwargs=hier_agent_kwargs, evaluate=evaluate)
        
    for ii in tqdm(range(n_sim)):
        task = task_list[ii]
        task.reset()
        results_mx[ii], _ = simulate_one(MetaAgent, ii, task, pruning_threshold=pruning_threshold,
