...
from cython_library import RewardHypothesis, MappingHypothesis, TransitionHypothesis, HierarchicalHypothesis
from cython_library import policy_iteration, value_iteration
from cython_library.assignments import *

from cython_library.core import value_iteration
...
#                    self.prune_hypothesis_space(threshold=pruning_threshold)
            ...
            if (t_prev != t) and isinstance(self,HierarchicalAgent):
                ...
                # log_priors_tuple = h_room.get_log_priors()
                # log_priors = {'rooms': [log_priors_tuple[0]],
                #               'env mappings': [log_priors_tuple[1]],
                #               'env rewards': [log_priors_tuple[2]],
                #               'room mappings': [log_priors_tuple[3]],
                #               'room rewards': [log_priors_tuple[4]]
                #     }
            ...
            if isinstance(self,HierarchicalAgent):
                ...
                # prev_log_priors_tuple = log_priors_tuple
                prev_rew_fn = rew_fn
                ...
                if ((xp, yp) in self.task.list_goal_locations):
                    ...
                    if len(clusterings['contexts']) == 1:
                        ...
                        # log_priors['rooms'].append(prev_log_priors_tuple[0])
                        # log_priors['env mappings'].append(prev_log_priors_tuple[1])
                        # log_priors['env rewards'].append(prev_log_priors_tuple[2])
                        # log_priors['room mappings'].append(prev_log_priors_tuple[3])
                        # log_priors['room rewards'].append(prev_log_priors_tuple[4])
                    ...
                    # log_priors['rooms'].append(log_priors_tuple[0])
                    # log_priors['env mappings'].append(log_priors_tuple[1])
                    # log_priors['env rewards'].append(log_priors_tuple[2])
                    # log_priors['room mappings'].append(log_priors_tuple[3])
                    # log_priors['room rewards'].append(log_priors_tuple[4])
                    
                    # print 'Goal', c, (xp, yp)
                    # print clusterings['rooms']
                    # print log_priors['rooms']
                    # print log_priors['env mappings']
                    # print log_priors['env rewards']
                    # print log_priors['room mappings']
                    # print log_priors['room rewards']
                    
                    sp = self.task.state_location_key[new_state]
                    print t, c, clusterings['state'][-1], clusterings['rew fn'][-1][sp], sp
                    print clusterings['rew fn'][-1]
                    # raw_input('asdf')
                
                if not (self.task.current_trial_number == t):
                    list_clusterings.append(clusterings)
                    # list_log_priors.append(log_priors)
                    
            ii += 1

        return pd.concat(results), list_clusterings, list_log_priors
...
class MetaAgent(ModelBasedAgent):
    ...
    def select_action(self, state):
        ...
        # p = self.get_joint_probability()
        # p_j = np.log(p)+max(self.joint_agent.log_belief)
        # p_i = np.log(1-p)+max(self.independent_agent.log_belief_rew)+max(self.independent_agent.log_belief_map)
        
        # if p_i > p_j or p == 0:
        #     return self.independent_agent.select_action(state)
        # else:
        #     return self.joint_agent.select_action(state)
...
class HierarchicalAgent(ModelBasedAgent):
    ...
    def get_transition_error(self, state):
        ...
        for aa in range(self.task.n_abstract_actions):
            p_aa = self.task.get_mapping_function(aa)
            for a in range(self.task.n_primitive_actions):
                q[t] = h_room.get_mapping_probability(c, a, aa)
                p[t] = p_aa[a]
                t += 1
    ...
    def evaluate_map_rewards(self, state):
        ...
        map_q_values = h_room.select_abstract_action_pmf(
            s, c, self.task.current_trial.transition_function
        )

        belief = np.exp(self.log_belief - np.max(self.log_belief))
        belief /= belief

        full_q_values = np.zeros(self.task.n_abstract_actions)
        for ii, p in enumerate(belief):
            h_room = self.rooms_hypotheses[ii]
            full_q_values += h_room.select_abstract_action_pmf(
                s, c, self.task.current_trial.transition_function
            ) * p
    ...
    def evaluate_map_mapping(self, state):
        ...
        map_mapping_mle = np.zeros(self.n_primitive_actions)
        for a0 in np.arange(self.n_primitive_actions, dtype=np.int32):
            map_mapping_mle[a0] = h_room.get_mapping_probability(c, a0, aa)
        ...
        for ii, p in enumerate(belief):
            h_room = self.rooms_hypotheses[ii]
            for a0 in np.arange(self.n_primitive_actions, dtype=np.int32):
                full_mapping_mle[a0] += h_room.get_mapping_probability(c, a0, aa) * p
