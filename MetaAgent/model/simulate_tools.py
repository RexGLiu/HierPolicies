...
def simulate_one(agent_class, simulation_number, task_kwargs, agent_kwargs=None, pruning_threshold=1000,
    ...
    if agent_class is HierarchicalAgent:
        results = agent.generate(evaluate=evaluate)
    else:
...
def simulate_mixed_task(n_sim, task_kwargs, agent_kwargs=None, alpha=2.0, pruning_threshold=1000,
    ...
    for ii in tqdm(range(n_sim)):
        ...
        results_ic[ii] = simulate_one(IndependentClusterAgent, ii, task_kwargs, agent_kwargs=agent_kwargs,
                                      pruning_threshold=pruning_threshold, evaluate=evaluate)
        results_fl[ii] = simulate_one(FlatControlAgent, ii, task_kwargs, pruning_threshold=pruning_threshold, agent_kwargs=agent_kwargs,
                                      evaluate=evaluate)
        results_h[ii] = simulate_one(HierarchicalAgent, ii, task_kwargs, agent_kwargs=hier_agent_kwargs, evaluate=evaluate)
    for ii in tqdm(range(n_sim)):
    ...
    return pd.concat([results_jc, results_ic, results_fl, results_mx, results_mx2, results_h])

def par_simulate_mixed_task(n_sim, task_kwargs, agent_kwargs=None, alpha=2.0, pruning_threshold=1000,
                        evaluate=False, seed=None,
                        meta_kwargs=None, metarl_kwargs=None):
    if agent_kwargs is None:
        agent_kwargs = dict(alpha=alpha)
    ...
    if seed is not None:
        np.random.seed(seed)
        rand_seeds = np.random.randint(np.iinfo(np.int32).max, size=(6,n_sim))
    ...
    inputs = tqdm(range(n_sim))
    results_jc = Parallel(n_jobs=num_cores)(delayed(simulate_one)(JointClustering, ii, task_kwargs, agent_kwargs=agent_kwargs,
                                      pruning_threshold=pruning_threshold, evaluate=evaluate, seed=rand_seeds[0,ii]) for ii in inputs)

    inputs = tqdm(range(n_sim))
    results_ic = Parallel(n_jobs=num_cores)(delayed(simulate_one)(IndependentClusterAgent, ii, task_kwargs, agent_kwargs=agent_kwargs,
                                      pruning_threshold=pruning_threshold, evaluate=evaluate, seed=rand_seeds[1,ii]) for ii in inputs)

    inputs = tqdm(range(n_sim))
    results_fl = Parallel(n_jobs=num_cores)(delayed(simulate_one)(FlatControlAgent, ii, task_kwargs, agent_kwargs=agent_kwargs,
                                      pruning_threshold=pruning_threshold, evaluate=evaluate, seed=rand_seeds[2,ii]) for ii in inputs)

    inputs = tqdm(range(n_sim))
    results_h = Parallel(n_jobs=num_cores)(delayed(simulate_one)(HierarchicalAgent, ii, task_kwargs, agent_kwargs=hier_agent_kwargs, 
                                      evaluate=evaluate, seed=rand_seeds[3,ii]) for ii in inputs)

    inputs = tqdm(range(n_sim))
    results_mx = Parallel(n_jobs=num_cores)(delayed(simulate_one)(MetaAgent, ii, task_kwargs, pruning_threshold=pruning_threshold,
                                      evaluate=evaluate, agent_kwargs=meta_kwargs, seed=rand_seeds[4,ii]) for ii in inputs)

    inputs = tqdm(range(n_sim))
    results_mx2 = Parallel(n_jobs=num_cores)(delayed(simulate_one)(RLMetaAgent, ii, task_kwargs, pruning_threshold=pruning_threshold,
                                       evaluate=evaluate, agent_kwargs=meta_kwargs, seed=rand_seeds[5,ii]) for ii in inputs)
