...
class Experiment(Task):
    ...
    def __init__(self, list_start_location, list_goals, list_context, list_subtask, list_action_map,
                 grid_world_size=(6, 6),
        ...
        self.list_subtask = list_subtask
        self.subtasks = set(list_subtask)
        self.n_subtasks = len(self.subtasks)
        
        self.subtask_goals = {}
        for ii in self.subtasks:
            subtask_goal_dict = list_goals[list_subtask.index(ii)]  # get goal dictionary for each subtask
            self.subtask_goals[ii] = set([g for g, _ in subtask_goal_dict.itervalues()])
            self.subtask_goal_index[ii] = {g: ii for ii, g in enumerate(self.subtask_goals[ii])}
            self.reverse_subtask_goal_index[ii] = {v: k for k, v in self.subtask_goal_index[ii].iteritems()}
        self.n_subtask_goals = len(list_goals[0])

        print 'goals: ', self.subtask_goals
    ...
    def get_goal_index(self, goal, subtask_idx = None):
        if subtask_idx is None:
            subtask_idx = self.list_subtask[self.current_trial_number]
        return self.subtask_goal_index[subtask_idx][goal]

    def get_goal_values(self):
        goal_values = np.zeros(self.n_subtask_goals)
        current_subtask = self.list_subtask[self.current_trial_number]
        current_goal_index = self.subtask_goal_index[current_subtask]
        for g, idx in current_goal_index():
    ...
    def get_current_goals(self):
        current_subtask = self.list_subtask[self.current_trial_number]
        return self.subtask_goals[current_subtask]
