...
def randomize_context_order_with_autocorrelation(balance, repeat_probability=0.25, n_subtasks=3):

    contexts = [(c,subtask_idx) for c, reps in enumerate(balance) for subtask_idx in range(n_subtasks) for _ in range(reps)]
    order = list()
