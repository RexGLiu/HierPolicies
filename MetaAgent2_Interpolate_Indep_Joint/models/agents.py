...
from misc import sample_cmf
from grid_world import GridWorld
from scipy.misc import logsumexp
from cython_library import GoalHypothesis, MappingHypothesis, HierarchicalHypothesis, policy_iteration, value_iteration
...
class MultiStepAgent(object):
    ...
    def resample_hypotheses(self, old_hypotheses, old_log_beliefs):
        ...
        if n_beliefs > self.n_particles:
            ...
            p = p/np.sum(p)
            arg_idx = np.random.choice(n_beliefs-self.n_max, self.n_particles-self.n_max, p=p, replace=False)
    ...
    def generate(self, evaluate=True, debug=False, pruning_threshold=None, use_map=False, comm=None):
        ...
        while True:
            ...
            if isinstance(self,HierarchicalAgent):
                results_dict['jointness'] = self.jointness
            else:
                results_dict['jointness'] = None
            ...
            if self.task.end_check():
                print rank, "Success", self.name
                break

            # stop criterion
            if step_counter > 2000:
                print rank, "Stop", self.name
                return None
...
class IndependentClusterAgent(FlatAgent):
    ...
    def resample_hypothesis_space(self):
        self.log_belief_goal, self.goal_hypotheses = self.resample_hypotheses(self.goal_hypotheses, 
                                    self.log_belief_goal)
...
class JointClusteringAgent(MultiStepAgent):
    ...
    def resample_hypothesis_space(self):
        ...
        if n_beliefs > self.n_particles:
            ...
            p = p/np.sum(p)
            arg_idx = np.random.choice(n_beliefs-self.n_max, self.n_particles-self.n_max, p=p, replace=False)
            new_goal_hypotheses = np.concatenate((new_goal_hypotheses, old_goal_hypotheses[arg_idx]))
            ...
            self.goal_hypotheses = new_goal_hypotheses
...
class MetaAgent(FlatAgent):
    ...
    def resample_hypothesis_space(self):
        self.joint_agent.resample_hypothesis_space()
        self.independent_agent.resample_hypothesis_space()
...
class HierarchicalAgent(MultiStepAgent):
    def __init__(self, task, inv_temp=10.0, alpha=1.0, jointness=0., gamma=0.8,
                 stop_criterion=0.001,
                 mapping_prior=0.001, goal_prior=0.001,
                 n_particles=300, p_max = 1, sample_tau = 1):
        super(HierarchicalAgent, self).__init__(task, n_particles=n_particles, p_max=p_max, sample_tau=sample_tau)

        self.name = "Hierarchical_"+str(jointness) 
        self.inverse_temperature = inv_temp
        self.gamma = gamma
        self.stop_criterion = stop_criterion
        self.current_trial = 0
        ...
        self.rooms_hypotheses = [HierarchicalHypothesis(self.task.n_goals, 
                    self.n_primitive_actions, self.n_abstract_actions, self.inverse_temperature, 
                    self.gamma, self.stop_criterion, alpha, jointness, goal_prior, mapping_prior)]
    
        self.log_belief = np.ones(1, dtype=float)
        self.jointness = jointness
        

    def update_mapping(self, c, a, aa):
        for ii, h_room in enumerate(self.rooms_hypotheses):
            assert type(h_room) is HierarchicalHypothesis
            h_room.updating_mapping(c, a, self.task.abstract_action_key[aa])
    ...
    def update_goal_values(self, c, goal, r):
        ...
        for ii, h_room in enumerate(self.rooms_hypotheses):
            assert type(h_room) is HierarchicalHypothesis
            h_room.update(c, goal_idx_num, r)
            self.log_belief[ii] = h_room.get_log_posterior()
    ...
    def augment_assignments(self, context):
        ...
        for h_room in new_hypotheses0:
            ...
            old_assignments = h_room.get_goal_assignments(context)
            new_assignments = augment_hierarchical_assignments([old_assignments], context)

            # create a list of the new clusters to add
            for assignment in new_assignments:
                h_r0 = h_room.deep_copy()
                h_r0.add_new_goal_context_assignment(context, assignment)
    ...
    #     n_beliefs = len(self.log_belief)
    #     if n_beliefs > self.n_particles:
    #         old_hypotheses = np.array(self.rooms_hypotheses)
    #         old_log_beliefs = np.array(self.log_belief)
    ...
    #         self.log_belief = new_log_beliefs
    #         self.rooms_hypotheses = new_hypotheses
    ...
    def get_goal_probability(self, context):
        ...
        goal_expectation = h_room.get_goal_probability(context)
    ...
    def get_mapping_function(self, context, aa):
        ...
        mapping_expectation = np.zeros(self.task.n_primitive_actions, dtype=float)
        for a0 in range(self.task.n_primitive_actions):
            mapping_expectation[a0] += h_room.get_mapping_probability(context, a0, aa)
    ...
    def get_action_pmf(self, location):
        ...
        # use softmax to convert to probability function
        p_aa = np.exp(self.inverse_temperature * q) / np.sum(np.exp(self.inverse_temperature * q))
        ...
        for aa0 in np.arange(self.task.n_abstract_actions, dtype=np.int32):
            ...
            _mapping_pmf = np.zeros(self.task.n_primitive_actions)
            for a0 in np.arange(self.task.n_primitive_actions, dtype=np.int32):
                _mapping_pmf[a0] = h_room.get_mapping_probability(c, a0, aa0)
            pmf += _mapping_pmf * p_aa[aa0]
