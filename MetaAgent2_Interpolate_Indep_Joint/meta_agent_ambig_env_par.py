...
def simulate_one(AgentClass, ii, task_args, task_kwargs, alpha, beta, name='None', jointness=0., flat=False, meta=False, hier=False, comm=None):
    if not flat:
        if hier:
    ...
    _res = None
    while _res is None:
        _res = agent.generate(evaluate=evaluate, pruning_threshold=pruning_threshold, comm=comm)
...
if rank == 0:
    ...
    list_jointness = np.arange(0.,1.2,0.2)
...
list_jointness = comm.bcast(list_jointness, root=0)
...
# Hierarchical Agent
results_hc = [[]]*len(list_jointness)
for kk in range(n_tasks):
    task_args, task_kwargs = list_tasks[kk]
    
    for ii, jointness in enumerate(list_jointness):
        _result = simulate_one(HierarchicalAgent, iteration_offset+kk, task_args, task_kwargs, 
                               alpha=1., beta=inv_temp, jointness=jointness, name='Hierarchical_'+str(ii), hier=True, comm=comm)
        results_hc[ii].append(_result)
...
for kk in range(n_tasks):
    ...
    _result = simulate_one(IndependentClusterAgent, iteration_offset+kk, task_args, task_kwargs, alpha, beta, name='Independent', comm=comm)
...
for kk in range(n_tasks):
    ...
    _result = simulate_one(JointClusteringAgent, iteration_offset+kk, task_args, task_kwargs, alpha, beta, name='Joint', comm=comm)
...
for kk in range(n_tasks):
    ...
    _result = simulate_one(FlatAgent, iteration_offset+kk, task_args, task_kwargs, alpha=None, beta=beta, name='Flat', flat=True, comm=comm)
...
for kk in range(n_tasks):
    ...
    _result = simulate_one(MetaAgent, iteration_offset+kk, task_args, task_kwargs, alpha, beta, name='Meta', meta=True, comm=comm)
...
if rank == 0:
    results_hc = [[]]*len(list_jointness)
    for _results in _results_hc:
        for ii, result in enumerate(_results):
            results_hc[ii] += result
    for ii, _results in enumerate(results_hc):
        results_hc[ii] = pd.concat(_results)
    results_hc = pd.concat(results_hc)
    results_hc.to_pickle("./Gen2GenEnvResults_hc.pkl")
    del _results_hc, _results, results_hc
...
if rank == 0:
    ...
    results_ic.to_pickle("./Gen2GenEnvResults_ic.pkl")
    del _results_ic, results_ic
...
if rank == 0:
    ...
    results_jc.to_pickle("./Gen2GenEnvResults_jc.pkl")
    del _results_jc, results_jc
...
if rank == 0:
    ...
    results_fl.to_pickle("./Gen2GenEnvResults_fl.pkl")
    del _results_fl, results_fl
...
if rank == 0:
    ...
    results_m.to_pickle("./Gen2GenEnvResults_m.pkl")
