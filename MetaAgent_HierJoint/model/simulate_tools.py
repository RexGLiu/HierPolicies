...
def par_simulate_mixed_task(n_sim, task_kwargs, agent_kwargs=None, alpha=2.0, pruning_threshold=1000,
    ...
    inputs = tqdm(range(n_sim))
    results_fl = Parallel(n_jobs=num_cores)(delayed(simulate_one)(FlatControlAgent, ii, task_kwargs, pruning_threshold=pruning_threshold,
                                      evaluate=evaluate, seed=rand_seeds[2,ii]) for ii in inputs)
