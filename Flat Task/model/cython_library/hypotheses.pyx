...
cdef class MappingHypothesis(object):
        ...
        cdef dict cluster_assignments, clusters, log_likelihoods
        ...
        def deep_copy(self):
            ...
            _h_copy.log_likelihoods = dict(self.log_likelihoods)
        ...
        def updating_mapping(self, int c, int a, int aa):
            ...
            self.log_likelihoods[k] = cluster.get_log_likelihood()

        def get_log_likelihood(self):
            return sum(self.log_likelihoods.values())
...
cdef class HierarchicalHypothesis(object):
    ...
    cpdef double get_log_posterior(self):
        cdef HierarchicalRoomCluster cluster
        cdef RewardCluster r_cluster
        cdef MappingCluster m_cluster

        cdef double log_posterior = self.room_log_prior + self.mapping_log_prior + self.reward_log_prior
        log_posterior += sum([cluster.get_reward_log_prior() + cluster.get_mapping_log_prior() for cluster in self.room_clusters.values()])
        log_posterior += sum([r_cluster.get_log_likelihood() for r_cluster in self.reward_clusters.values()])
    ...
    def get_all_reward_assignments(self):
        cdef HierarchicalRoomCluster cluster

        cdef list reward_assignments = [self.reward_assignments, [cluster.get_reward_assignments() for cluster in self.room_clusters.values()]]
        
        return reward_assignments

    def get_all_mapping_assignments(self):
        cdef HierarchicalRoomCluster cluster

        cdef list mapping_assignments = [self.mapping_assignments, [cluster.get_mapping_assignments() for cluster in self.room_clusters.values()]]
    ...
    def add_new_room_context_assignment(self, int c, int k):
        ...
        if k not in self.room_clusters.keys():
            # if not, add an new room cluster
            self.room_clusters[k] = HierarchicalRoomCluster(self.n_stim, self.n_primitive_actions, 
                              ...
                              self.iteration_criterion, self.alpha0, self.alpha1)
...
cdef class HierarchicalRoomCluster(object):
    ...
    cdef double gamma, iteration_criterion, inverse_temperature, alpha0, alpha1
    cdef dict room_assignments, room_clusters, reward_assignments, reward_clusters, mapping_assignments, mapping_clusters
    cdef double room_log_prior, reward_log_prior, mapping_log_prior
