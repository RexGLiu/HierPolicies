...
class MultiStepAgent(object):
    ...
    def __init__(self, task, n_particles=100, sample_tau = 1):
        self.task = task
        assert type(self.task) is Task
        self.current_trial = 0

        self.n_particles = n_particles
        self.n_max = self.n_particles
    ...
    def resample_hypotheses(self, old_hypotheses, old_log_beliefs):
        ...
        if n_beliefs > self.n_particles:
            ...
            if n_beliefs > self.n_bottleneck:
                # first randomly sample n_bottleneck particles
                arg_idx = np.random.choice(n_beliefs, self.n_bottleneck, replace=False)
                old_hypotheses = old_hypotheses[arg_idx]
                old_log_beliefs = old_log_beliefs[arg_idx]
            
            # then select the n_max MAP hypotheses from the sampled particles
...
#                if times_seen_ctx[c] == 1:
#
#                    if not isinstance(self,HierarchicalAgent):
#                        self.prune_hypothesis_space(threshold=pruning_threshold)
#
##                    if isinstance(self,HierarchicalAgent):
##                        self.resample_hypothesis_space()
...
class JointClustering(ModelBasedAgent):
    ...
    def __init__(self, task, inverse_temperature=100.0, alpha=1.0,  discount_rate=0.8, iteration_criterion=0.01,
                 mapping_prior=0.01, n_bottleneck=10000):
...
class IndependentClusterAgent(ModelBasedAgent):
    ...
    def __init__(self, task, inverse_temperature=100.0, alpha=1.0, discount_rate=0.8,
                 iteration_criterion=0.01, mapping_prior=0.01, n_bottleneck=10000):
...
class MetaAgent(ModelBasedAgent):
    ...
    def __init__(self, task, inverse_temperature=100.0, alpha=1.0, discount_rate=0.8,
                 ...
                 mapping_prior=0.01, m_biases=[0.0, 0.0], n_bottleneck=10000):
        assert type(task) is Task
        super(FullInformationAgent, self).__init__(task)
        ...
        self.independent_agent = IndependentClusterAgent(
            task, inverse_temperature=inverse_temperature,  alpha=alpha, discount_rate=discount_rate,
                 iteration_criterion=iteration_criterion, mapping_prior=mapping_prior, n_bottleneck=n_bottleneck
        )
...
class RLMetaAgent(ModelBasedAgent):
    ...
    def __init__(self, task, inverse_temperature=100.0, alpha=1.0, discount_rate=0.8,
                 iteration_criterion=0.01,
                 mapping_prior=0.01, m_biases=[0.0, 0.0], mixing_lrate=0.5, mixing_temp=1.0, n_bottleneck=10000):
        assert type(task) is Task
        super(FullInformationAgent, self).__init__(task)
