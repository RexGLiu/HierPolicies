...
cdef class GoalCluster(object):
    cdef double goal_prior
    cdef double [:]  goal_reward_probability
    ...
    def update(self, int goal, int r):
        ...
        if r == 0:
            ...
            self.goal_reward_probability[goal] = 1E-7  # to avoid log(0) when computing log_likelihoods
            self.goal_reward_probability /= np.sum(self.goal_reward_probability)
        else:
            self.goal_visits[goal,1] += 1
            self.goal_reward_probability[:] = 1E-7  # to avoid log(0) when computing log_likelihoods
            self.goal_reward_probability[goal] = 1 - 1E-7*(self.n_goals-1)  # second term ensures probs are properly normalised
...
cdef class HierarchicalHypothesis(object):
    ...
    def get_door_seq_hierarchy(self, int c):
        ...
        cdef UpperRoomCluster cluster = self.upper_room_clusters[k]
        return [self.door_seq_assignments] + cluster.get_door_seq_hierarchy()

    def get_complete_door_seq_hierarchy(self):
        cdef UpperRoomCluster cluster
        return [self.door_seq_assignments, [ cluster.get_door_seq_hierarchy() for cluster in self.upper_room_clusters.values() ] ]
